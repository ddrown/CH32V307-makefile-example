...
// USB interrupt
void OTG_FS_IRQHandler() __attribute__((naked));
...
void USBFS_Device_Init( FunctionalState sta )
{
    if( sta )
    {
        ...
        SetVTFIRQ((u32)OTG_FS_IRQHandler,OTG_FS_IRQn,0,ENABLE);
    }
    else
    {
        USBOTG_H_FS->BASE_CTRL = USBFS_UC_RESET_SIE | USBFS_UC_CLR_ALL;
 ...
 * ^ WCH-Interrupt-fast is not in xpack gcc, alternate
 * is this any faster than just attribute(interrupt)?
 */
void OTG_FS_IRQHandler() {
  __asm volatile ("call OTG_FS_IRQHandler_real; mret");
}
 ...
 * @return  none
 */
__attribute__ ((used)) void OTG_FS_IRQHandler_real( void )
{
...
void USB_Print_stats() {
    printf("IN=%u OUT=%u SETUP=%u SOF=%u RST=%u SUS=%u OTHER=%u IRQ=%u\r\n", stats[0], stats[1], stats[2], stats[3], stats[4], stats[5], stats[6], stats[7]);
}
